...
def seqs_to_ids(seqs, char_to_id, max_len=20):
    ...
    for text in seqs:
        if len(text) != 0:
            all_text = []
            for word in text:
                all_text.append(char_to_id.get(word))

        all_seqs.append(all_text)
    final_all_seqs = np.zeros((len(all_seqs), max_len))

    for idx, row in enumerate(all_seqs):
        if len(row) >= max_len:
            final_all_seqs[idx, :max_len] += row[:max_len]
        else:
            final_all_seqs[idx, :len(row)] += row
    print('Shape of data tensor:', final_all_seqs.shape)

    return final_all_seqs
...
def train_model(model, Xtrain, Ytrain, Xval, Yval, id_to_char, max_epoch):
    ...
    for e in range(max_epoch):
        for batch in range(0, num_batches):
            ...
            #Initializing model state and forward loop
            #h_last = model.__init__(Xbatch.shape[0])
            out, h = model(Xbatch)

            #Calculating loss
            loss = lossfn(out.permute(0, 2, 1), Ybatch)
...
def gen_string(model, id_to_char, max_len=20, sample=True):
    ...
    for i in range(max_len):
        ...
        # The generation loop of the RNNLM model
        #       this should generate a name from the model
        #       using either sampling or argmax decoding
        out, h_last = model(x, h)

        # calculate prob
        y_pred = nn.functional.softmax(out, 2)

        # switch between random sampling and argmax
        if sample:
            idx = np.random.choice(range(0, len(id_to_char)), p=y_pred.data.numpy().ravel())
        else:
            idx = np.argmax(y_pred.detach().numpy())

        ch = id_to_char[idx]

        if(ch == '.'):
            break
        # Appending the character to output string
        out_str += ch

        #Assigning the variables for the next iteration
        x = torch.tensor(np.array(idx).reshape(1,1))
        h = h_last
